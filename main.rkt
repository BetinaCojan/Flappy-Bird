...
(define-struct my-bird (x y speedY) #:transparent)
(define-struct my-pipe (gapX gapY gapH) #:transparent) 
(define-struct my-variables (gravity momentum scroll-speed) #:transparent)
(define-struct my-score (x y nr) #:transparent)
;BONUS
;(define-struct my-abilities (image time posX posY next visible active) #:transparent)
(define-struct my-abilities (visible active) #:transparent)

(define-struct my-state
  (my-bird pipes my-variables my-abilities my-score chenarX chenarY) #:transparent)

(define bird (my-bird bird-x bird-initial-y 0)) 
(define pipe (my-pipe scene-width (+ added-number (random random-threshold)) pipe-self-gap))
(define variables (my-variables initial-gravity initial-momentum initial-scroll-speed))
(define abilities (my-abilities 0 0))
(define score (my-score text-x text-y 0))
(define pipes (list pipe))
        ; lista de pipe-uri // initial contine doar X ul primului pipe
(define state (my-state bird pipes variables abilities score 0 0)) ; am dat val initiale 0 pt chenar
...
(define (next-score score) ; functie auxiliara pt next-state-score
  (match-let ([(my-score x y nr) score])
    (struct-copy my-score score [nr (+ nr 0.1)])))
#|
; functie auxiliara pt a da val initiala 0 lui score in initial-state
(define (create-initial-score score)
   (match-let ([(my-score _ _ nr) score])
    (struct-copy my-score score [nr 0])))

(define (get-initial-state)
  (match-let ([(my-state _ _ _ _ my-score _ _ ) state])
    (create-initial-score score)))
  
  ;(create-initial-score (get-score state)))
|#
...
(define (next-state-bird bird gravity)
  (match-let ([(my-bird _ y speedY) bird])
    (struct-copy my-bird bird [y (+ y speedY)] [speedY (+ speedY gravity)])))
...
(define (next-state-bird-onspace bird momentum)
  (match-let ([(my-bird _ _ speedY) bird])
    (struct-copy my-bird bird [speedY (- speedY momentum)])))
...
(define (change current-state pressed-key)
  (match-let ([(my-state my-bird _ _ _ _ _ _) current-state])
    (if (key=? pressed-key " ")
        (struct-copy my-state current-state
                     [my-bird
                      (next-state-bird-onspace (get-bird current-state) initial-momentum)])
        current-state)))
...
;functie folosita pentru a modifica x-ul dintr-o struct pipe
(define (sub-speed pipe scroll-speed)
  (match-let ([(my-pipe gapX _ _) pipe])
    (struct-copy my-pipe pipe [gapX (- gapX scroll-speed)])))
         
(define (move-pipes pipes scroll-speed)
  (if (null? pipes)
      pipes
      (cons (sub-speed (car pipes) scroll-speed) (move-pipes (cdr pipes) scroll-speed))))
...
(define (clean-pipes pipes)
  (cond
    ((null? pipes) pipes)
    ((is-over? (car pipes)) (clean-pipes (cdr pipes)))
    (else pipes)))
  ...
  (if (< (length pipes) no-pipes)
       (append pipes
        (list (make-my-pipe
               ...
               pipe-self-gap)))
      pipes))
; am adaugat la lista de pipes un nou pipe
; pe care l-am creat cu functia make-id_struct
...
(define (get-scrore-from state)
  (my-state-my-score state))

(define (get-score state)
  (my-score-nr (get-scrore-from state)))

;(get-score (next-state initial-state))
...
(define (check-ground-collision bird)
 (match-let ([(my-bird _ y _) bird])
   (if (>= (+ y bird-height) ground-y)
       #t
       #f)))
...
(define (check-pipe-down-collisions bird pipe)
  (match-let ([(my-bird x y _) bird]
              [(my-pipe gapX gapY gapH) pipe])
    (and (< (my-bird-x bird) (+ (my-pipe-gapX pipe) pipe-width))
         (> (+ (my-bird-x bird) bird-width) (my-pipe-gapX pipe))
         (< (my-bird-y bird) ground-y)
         (> (+ (my-bird-y bird) bird-height) (my-pipe-gapY pipe)))))

(define (check-pipe-up-collisions bird pipe)
  (match-let ([(my-bird x y _) bird]
              [(my-pipe gapX gapY gapH) pipe])
    (and (< (my-bird-x bird) (+ (my-pipe-gapX pipe) pipe-width))
         (> (+ (my-bird-x bird) bird-width) (my-pipe-gapX pipe))
         (< (my-bird-y bird) (my-pipe-gapY pipe))
         (> (+ (my-bird-y bird) bird-height) 0))))
  
  
(define (check-pipe-collisions bird pipes)
  (if (null? pipes)
      #f
      (or (check-pipe-up-collisions bird (car pipes))
          (check-pipe-down-collisions bird (car pipes))
...
(define (next-state state)
  (match-let ([(my-state my-bird pipes _ _ my-score _ _) state])
    (struct-copy my-state state
                 [my-bird (next-state-bird bird initial-gravity)]
                 [pipes (next-state-pipes pipes initial-scroll-speed)]
                 [my-score (next-score score)])))
...
(define get-variables-gravity (my-variables-gravity variables))
(define get-variables-momentum (my-variables-momentum variables))
(define get-variables-scroll-speed (my-variables-scroll-speed variables))
